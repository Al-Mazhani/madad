...
class ControllBook extends BaseController
{
    ...
    public function __construct($model, $AuthorModel, $CategoryModel = null)
    {
    ...
    private function validateFileInputBook($dataAddBook)
    {
        ...
        if ($dataAddBook['book']['size'] > (8 * 1024 * 1024)) { // Max Size Of File Is 8MB
            ...
            return ['hasFileEmpty' => 'حجم الكتاب كبير يجب ان يكون اقل من  8MB'];
        }
        if (!$this->CheckAllowedMimeTypeFile($dataAddBook['book'], 'book')) {
    ...
    private function validateFileInputImage($dataAddBook)
    {
        ...
        if ($dataAddBook['image']['size'] > (8 * 1024 * 1024)) { // Max Size Of File Is 8MB
            ...
            return ['hasFileEmpty' => 'حجم الصورة كبير يجب ان يكون اقل من  8MB'];
        }

        if (!$this->CheckAllowedMimeTypeFile($dataAddBook['image'], 'image')) {
    ...
    {
        if (!$this->AuthorModel->checkIDExit($IDs['id_author'])) {
    ...
    private function validateBook($dataAddBook)
    {
        ...
        if ($this->model->CheckTitleBookExit($dataAddBook['nameBook'])) {
    ...
    {
        return $this->model->loadCategory();
    }
    ...
    public function getBookAuthor($id)
    {
        ...
        $this->validateID($id);
        $resultBookWithAuthor = $this->model->loadBookByAuthorID($this->CleanInputText($id));
    ...
    public function updateBook($id, $dataUpdateBook)
    {
        ...
        $resultUpdate = $this->model->update($id, $dataUpdateBook);
        $FileCacheName = __DIR__ . '/../cache/baseBook.json';
    ...
    public function addBook($dataAddBook)
    {
        ...
        $result = $this->model->insertBook($dataAddBook);
    ...
    {
        $this->validateID($id);
        $resultGetBookByCategory = $this->model->loadBookByCateogryID($this->CleanInputText($id));
    ...
    public function getInfoBookByID($idBook)
    {
        ...
        $resultInfoBook = $this->model->infoBook($this->CleanInputNumber($idBook));
    ...
    public function incrementDonwnload($id)
    {
        ...
        $resultDownload = $this->model->incrementDonwnload($id);
        return ($resultDownload) ? "تم تنزيل الكتاب" : "فشل في تنزيل الكتاب";
    }
    public function incrementReadBook($id)
    {

        $this->model->incrementReadBook($this->CleanInputText($id));
    }
    public function OtherBooks()
    {
        $OtherBooks = $this->model->LoadOtherBooks();
        if (empty($OtherBooks)) {
            return [];
        }
        return $OtherBooks;
